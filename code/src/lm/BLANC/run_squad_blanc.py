...
def evaluate(args, model, device, eval_dataset, eval_dataloader,
    ...
    if args.version_2_with_negative:
        ...
        exact_raw, f1_raw, _, _, span_exact, span_f1, span_p, span_r = get_raw_scores(eval_dataset, preds, eval_examples)
...
def main(args):
    ...
    if args.do_train:
        ...
        for lr in lrs:
            model, state_dict = BLANC.from_pretrained(
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_batches):
                    ...
                    loss, _ = model(input_ids, segment_ids, input_mask, start_positions, end_positions, args.geometric_p, args.window_size, lmb=args.lmb)
                    if n_gpu > 1:
                        loss = loss.mean()
                    if args.gradient_accumulation_steps > 1:
                    ...
                    train_time += (train_time_e - train_time_s)
                    if (step + 1) % eval_step == 0:
                        logger.info('Epoch: {}, Step: {} / {}, used_time = {:.2f}s, train_time = {:.6f}'.format(
                            epoch, step + 1, len(train_batches), time.time() - start_time, train_time))
                        train_time = 0.0
    ...
    if args.do_eval:
        if args.eval_test:
            ...
            eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_example_index)
            eval_dataloader = DataLoader(eval_data, batch_size=args.eval_batch_size)
        model, state_dict = BLANC.from_pretrained(args.output_dir)
        if args.fp16:
            model.half()
        model.to(device)
        na_prob_thresh = 1.0
        ...
        with open(os.path.join(args.output_dir, "test_results.json"), "w") as writer:
            for key in sorted(result.keys()):
                writer.write("%s = %s\n" % (key, str(result[key])))
